    ...
    try:
        if settings['auto_delete']:
            btn.insert(
                0,
                [
                    InlineKeyboardButton(f' ɪɴꜰᴏ', 'minfo'),
            ...
            btn.insert(
                0,
                [
                    InlineKeyboardButton(f' ɪɴꜰᴏ', 'minfi'),
                    InlineKeyboardButton("ꜰᴏʀᴍᴀᴛ​", 'reqinfo'),
                    InlineKeyboardButton(f'ᴛɪᴘs​ ⚜', 'sinfo'),
                ],
            )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if ENABLE_SHORTLINK == True:
        if settings["button"]:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"📁[{get_size(file.file_size)}] ✦{file.file_name}", url=await get_shortlink(message.chat.id, f"https://telegram.me/{clonedme.U_NAME}?start=files_{file.file_id}")
                    ),
                ]
                for file in files
            ]
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"{file.file_name}",
                        url=await get_shortlink(message.chat.id, f"https://telegram.me/{clonedme.U_NAME}?start=files_{file.file_id}")
                    ),
                    InlineKeyboardButton(
                        text=f"{get_size(file.file_size)}",
                        url=await get_shortlink(message.chat.id, f"https://telegram.me/{clonedme.U_NAME}?start=files_{file.file_id}")
    ...
    if imdb:
        ...
        cap = f"<b>Hᴇʏ {message.from_user.mention}\n\n📁𝙵ᴏᴜɴᴅ ✨️ 𝙵ɪʟᴇꜱ 𝙵ᴏʀ 𝚈ᴏᴜʀ 𝚀ᴜᴇʀy {search}.</b>"
    if imdb and imdb.get('poster'):
        try:
            if message.chat.id == SUPPORT_CHAT_ID:
            ...
            if message.chat.id == SUPPORT_CHAT_ID:
                ...
                hmm = await message.reply_text(text=cap, reply_markup=InlineKeyboardMarkup(btn))
                try:
                    if settings['auto_delete']:
                        await asyncio.sleep(600)
                        await hmm.delete()
